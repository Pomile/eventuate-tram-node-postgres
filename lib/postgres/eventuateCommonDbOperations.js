...
function insertIntoMessageTable (messageId, payload, destination, currentTimeInMillisecondsSql = new Date().getTime(), headers = {}, context = {}) {
  ...
  if (trx) {
    return knex(MESSAGE_TABLE).withSchema('eventuate').transacting(trx).insert(message);
  }

  return knex(MESSAGE_TABLE).withSchema('eventuate').insert(message);
}

async function getMessageById(id) {
  const [ message ] = await knex(MESSAGE_TABLE).withSchema('eventuate').where('id', id);
  return message;
}
